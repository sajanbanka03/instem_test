...
public class TestUtil extends BaseClass{
	...
	public static void datePicker(WebElement calender, WebElement calNextButton, WebElement calprevButton, WebElement calmidButton, String date ) throws Exception
	{
        calender.click();
        ...
        if(Integer.parseInt(dateSelect[1])>=1 && Integer.parseInt(dateSelect[1])<=12)
        {        
        	months.get(Integer.parseInt(dateSelect[1])-1).click();
        }
        else{
        	Reporter.log("Invalid month provided. please provide correct time.");
        }
        
        Thread.sleep(1500);
        List<WebElement> dates = driver.findElements(By.xpath("xpath for dates using td"));
        if(Integer.parseInt(dateSelect[0])>=1 && Integer.parseInt(dateSelect[0])<=31)
        {        
        	dates.get(Integer.parseInt(dateSelect[0])-1).click();
        }
        else
        {
        	Reporter.log("Invalid date provided. Please provide correct date");
        }
